...
class _RoomCodeScreenState extends State<RoomCodeScreen> {
  ...
  Future<void> _join() async {
    ...
    try {
      await apiService.post(ApiConstants.joinRoom, data: {'roomCode': code});
      _show('加入成功！');
      _load();
    } catch (_) { _show('加入失败，请检查房间码'); }
      ...
      child: ListView(padding: const EdgeInsets.only(bottom: 28), children: [
        ...
        if (_myRoomCode != null) ...[
          Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: AppCard(color: AppTheme.peach, child: Column(children: [
            const Text('你的房间码', style: TextStyle(color: AppTheme.textGray)),
            ...
            OutlinedButton.icon(
              ...
              icon: const Icon(Icons.copy), label: const Text('复制房间码'),
            ),
          ]))),
        ...
        Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: AppCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('📥 加入 TA 的房间', style: TextStyle(fontWeight: FontWeight.w700)),
          ...
          const SizedBox(height: 16),
          SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _loading ? null : _join, child: _loading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('加入房间'))),
        ]))),
      ]),
    ),
  );
