...
class _WeatherScreenState extends State<WeatherScreen> {
  ...
  Future<void> _fetch({String? city, double? lat, double? lng}) async {
    ...
    try {
      ...
      setState(() {
        _cityName = cityName.isNotEmpty ? cityName : '当前位置';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: ListView(padding: const EdgeInsets.only(bottom: 28), children: [
        AppGradientHeader(
          title: _cityName.isNotEmpty ? '${_cityName} 天气' : '天气',
          subtitle: _loading ? '定位中...' : '搜索其他城市查看天气',
          icon: '🌈',
        ),
        const SizedBox(height: 14),
        Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(children: [
          Expanded(child: TextField(controller: _cityCtrl, decoration: const InputDecoration(hintText: '输入城市名', prefixIcon: Icon(Icons.location_city_outlined)), onSubmitted: (v) => _fetch(city: v))),
        ...
        if (_error != null) Padding(padding: const EdgeInsets.all(20), child: Center(child: Text(_error!, style: const TextStyle(color: Colors.redAccent)))),
        if (!_loading && _error == null) ...[
          if (now != null)
            Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: AppCard(color: AppTheme.sky, child: Column(children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                _stat('体感', '${now['feelsLike'] ?? '--'}°'), _stat('湿度', '${now['humidity'] ?? '--'}%'), _stat('风力', now['windDir']?.toString() ?? '--'),
              ]),
            ]))),
          if (daily.isNotEmpty) ...[
            const SizedBox(height: 14),
            const Padding(padding: EdgeInsets.symmetric(horizontal: 20), child: AppSectionTitle(title: '三日预报')),
            ...
            SizedBox(height: 120, child: ListView.separated(
              ...
              itemCount: daily.length, separatorBuilder: (_, __) => const SizedBox(width: 10),
              itemBuilder: (_, i) {
                final d = daily[i] as Map<String, dynamic>;
                return SizedBox(width: 140, child: AppCard(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          if (_reminder != null && _reminder!.isNotEmpty) ...[
            const SizedBox(height: 14),
            Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: AppCard(color: AppTheme.lightPink, child: Row(children: [
  ...
  Widget _stat(String l, String v) => Column(children: [Text(v, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)), Text(l, style: const TextStyle(fontSize: 12, color: AppTheme.textGray))]);
}
